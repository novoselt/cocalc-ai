...
describe("conat child process lifecycle", () => {
  ...
  it("installs forwarding handlers only after forking a child", () => {
    ...
    try {
      jest.isolateModules(() => {
        const { forkedConatServer } = require("./start-server");
        forkedConatServer({ port: 1234 });
      });
      expect(fork).toHaveBeenCalledTimes(1);
      expect(child.send).toHaveBeenCalledWith({ port: 1234 });
      for (const signal of SIGNALS) {
