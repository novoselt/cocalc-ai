...
# Thin wrapper. The actual TeX Live install/update logic, the system
# configuration and the test documents live in a separate repository, so they
# stay ordinary shell files and real fixtures rather than heredocs embedded in
# a recipe. This module only clones that repository at a pinned ref and hands
# over, passing the recipe inputs through as environment variables.
