...
const logger = getLogger("project-host:jupyter-ipynb");
...
async function loadBlob(project_id: string, uuid: string) {
...
async function importIpynb(
  ...
  const liveIpynb = hasNativeAttachments(ipynb)
    ? await externalizeJupyterAttachments({
        ipynb,
        loadBlob: async (uuid) => await loadBlob(project_id, uuid),
...
export async function importJupyterIpynb({
  ...
  return await withNotebookAdmission(
    project_id,
    bytes,
    async () => await importIpynb(project_id, ipynb, true),
  );
}
...
export async function saveJupyterIpynb({
  ...
  return await withNotebookAdmission(project_id, inputBytes, async () => {
    const { ipynb: liveIpynb } = await importIpynb(project_id, ipynb, true);
