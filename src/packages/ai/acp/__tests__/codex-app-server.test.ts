...
describe("CodexAppServerAgent", () => {
  ...
  it("warns when invisible notifications mask a silent running turn", async () => {
    const proc = new FakeCodexAppServerProc((fake, message) => {
      switch (message.method) {
        case "initialize":
          fake.sendResponse(message.id, { ok: true });
          break;
        case "thread/start":
          fake.sendResponse(message.id, { thread: { id: "thr-silent" } });
          break;
        case "turn/start":
          fake.sendResponse(message.id, { turn: { id: "turn-silent" } });
          setImmediate(() => {
            fake.sendNotification("turn/started", {
              turn: { id: "turn-silent", status: "inProgress" },
            });
            const metadataTimer = setInterval(() => {
              fake.sendNotification("thread/status/changed", {
                threadId: "thr-silent",
                turnId: "turn-silent",
              });
            }, 10);
            setTimeout(() => {
              ...
              fake.sendNotification("turn/completed", {
                turn: { id: "turn-silent", status: "completed" },
              });
            }, 120);
          });
          break;
    ...
    const agent = new CodexAppServerAgent({ visibleActivityWarningMs: 40 });
    const streamPayloads: any[] = [];
    await agent.evaluate({
      project_id: "00000000-0000-4000-8000-000000000000",
      account_id: "00000000-0000-4000-8000-000000000001",
    ...
    expect(loggerMock.warn).toHaveBeenCalledWith(
      "codex app-server: no visible turn activity",
      expect.objectContaining({
        threadId: "thr-silent",
        turnId: "turn-silent",
        notificationMethods: expect.objectContaining({
          "thread/status/changed": expect.any(Number),
        }),
      }),
    );
  });
