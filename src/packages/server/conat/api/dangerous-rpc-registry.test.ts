...
const DANGEROUS_RPC_NAME_PATTERN =
  /^(?:add|admin|apply|approve|archive|assign|backup|begin|bootstrap|cancel|claim|clear|cleanup|create|delete|diagnostic|disable|dismiss|drain|exec|finalize|force|gc|generate|hard|import|issue|leave|logs|mark|move|open|prepare|prune|publish|pull|purchase|purge|query|reconcile|record|release|remove|repair|request|reserve|restart|restore|rehome|review|revoke|rollout|rotate|run|save|scan|send|set|start|stop|sync|terminate|update|upgrade|upsert)/i;
