...
/**
 * Remove every `% chat: <hash>` marker for the given hash from `text`.
 * Block-form marker lines are removed entirely; inline markers strip the
 * comment tail (including trailing whitespace before the `%`).
 */
export function removeMarkersForHash(text: string, hash: string): string {
  const lines = text.split("\n");
  const targets = scanMarkers(text)
    .filter((m) => m.hash === hash)
    .sort((a, b) => b.line - a.line);
  for (const t of targets) {
    const lineText = lines[t.line];
    if (lineText == null) continue;
    const before = lineText.slice(0, t.col);
    if (before.trim() === "") {
      lines.splice(t.line, 1);
    } else {
      lines[t.line] = before.replace(/\s+$/, "");
    }
  }
  return lines.join("\n");
}
