...
import { getEdgeEndpoints, getRectAnchorToward } from "./math";
...
describe("getEdgeEndpoints", () => {
  it("connects bottom to top for vertically stacked rects", () => {
    ...
    const { start, end, startSide, endSide } = getEdgeEndpoints(a, b);
  ...
  it("connects top to bottom when the target is above", () => {
    ...
    expect(startSide).toBe("top");
    expect(endSide).toBe("bottom");
  });
  ...
  it("returns a side and boundary point for a free target point", () => {
    const anchor = getRectAnchorToward(r(10, 20, 100, 60), {
      x: 500,
      y: 40,
    });
