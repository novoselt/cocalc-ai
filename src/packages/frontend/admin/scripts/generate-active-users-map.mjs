...
const COUNTRIES_110M_URL = `${NATURAL_EARTH_ROOT}/ne_110m_admin_0_countries.geojson`;
...
function renderMap(countries) {
  const countryPaths = countries.features
    .map(
      ({ geometry }) =>
        `  <path d="${geometryPath(geometry)}" fill-rule="evenodd"/>`,
    )
    .join("\n");
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${ACTIVE_USERS_MAP.width} ${ACTIVE_USERS_MAP.height}" role="img" aria-labelledby="title description" data-projection="${ACTIVE_USERS_MAP.projection}">
  ...
  <path d="${globePath()}" fill="${COLORS.BLUE_LLLL}" stroke="${COLORS.BLUE_LL}" stroke-width="1.5"/>
  <g fill="none" opacity="0.45" stroke="${COLORS.BLUE_LL}" stroke-width="0.75">
${graticule}
  </g>
  <g fill="${COLORS.GRAY_LLL}" stroke="${COLORS.GRAY}" stroke-linejoin="round" stroke-width="0.65">
${countryPaths}
  </g>
</svg>
`;
...
const [countries110m, countries50m, mapUnits50m] = await Promise.all([
  getGeoJson(COUNTRIES_110M_URL),
